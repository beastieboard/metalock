...
use crate::{expr::Function, parse::{rdd, R}, schema::tag::{self, TagType}, tlist, types::{impl_deref, impl_into, tlist::*, Buffer, Decode, EncodedFunction, Parser, ParserBuffer, Schema, RD}};
...
pub fn data_parse(buf: Parser) -> R<RD> {
    ...
    fn parse_inner(schema: Parser, data: Parser) -> R<RD> {
        ...
        Ok(match tag {
            ...
            // TODO: Zero copy?
            tag::BUF32::ID => data.take::<32>()?.into(),
            tag::OPTION::ID => {
